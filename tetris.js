...
rtl.module("utetris",["System","Classes","SysUtils","Web"],function () {
  ...
  rtl.recNewT($mod,"TCoordinate",function () {
    this.x = 0;
    this.y = 0;
    this.$eq = function (b) {
  ...
  this.TBlock$clone = function (a) {
    var r = [];
    for (var i = 0; i < 4; i++) r.push($mod.TCoordinate.$clone(a[i]));
    return r;
  };
  this.TBlocks$clone = function (a) {
    var r = [];
    for (var i = 0; i < 7; i++) r.push($mod.TBlock$clone(a[i]));
    return r;
  };
  rtl.createClass($mod,"TTetris",pas.Classes.TComponent,function () {
    this.$init = function () {
      pas.Classes.TComponent.$init.call(this);
      this.FCanvasID = "";
    ...
    this.DoMouseClick = function (aEvent) {
      ...
      if (pas.System.Copy(S,1,SControl.length) === SControl) {
        ...
        if ($tmp1 === "left") {
          ...
          this.RotateBlock()}
         else if ($tmp1 === "drop") this.DropBlock();
      };
      return Result;
    };
    ...
    this.EnableTick = function () {
      ...
      this.FMyInterval = window.setInterval(rtl.createCallback(this,"CheckBlockDown"),this.FInterval);
    };
    ...
    this.MoveAllRowsDown = function (rowsToDelete, startOfDeletion) {
      ...
      var y = 0;
      var Dest = 0;
      for (var $l1 = startOfDeletion - 1; $l1 >= 0; $l1--) {
        I = $l1;
        for (x = 0; x <= 11; x++) {
          y = I + rowsToDelete;
    ...
    this.RecalcScore = function (aRows) {
      ...
      this.DrawScore();
      newLevel = 1 + Math.floor(this.FScore / this.FIncLevelScore);
    ...
    this.SetupTetris = function () {
      ...
      for (i = 1; i <= 5; i++) {
        ...
        if (el != null) el.onclick = rtl.createCallback(this,"DoMouseClick");
      };
      if (this.FBtnReset != null) this.FBtnReset.onclick = rtl.createCallback(this,"DoResetClick");
      ...
      document.onkeydown = rtl.createCallback(this,"HandleKeyPress");
    };
    ...
    this.CreateBlocks = function () {
      ...
      $Self.FBlocks[0] = [$mod.TCoordinate.$clone(co(1,0)),$mod.TCoordinate.$clone(co(0,1)),$mod.TCoordinate.$clone(co(1,1)),$mod.TCoordinate.$clone(co(2,1))];
      $Self.FBlocks[1] = [$mod.TCoordinate.$clone(co(0,0)),$mod.TCoordinate.$clone(co(1,0)),$mod.TCoordinate.$clone(co(2,0)),$mod.TCoordinate.$clone(co(3,0))];
      $Self.FBlocks[2] = [$mod.TCoordinate.$clone(co(0,0)),$mod.TCoordinate.$clone(co(0,1)),$mod.TCoordinate.$clone(co(1,1)),$mod.TCoordinate.$clone(co(2,1))];
      $Self.FBlocks[3] = [$mod.TCoordinate.$clone(co(0,0)),$mod.TCoordinate.$clone(co(1,0)),$mod.TCoordinate.$clone(co(0,1)),$mod.TCoordinate.$clone(co(1,1))];
      $Self.FBlocks[4] = [$mod.TCoordinate.$clone(co(2,0)),$mod.TCoordinate.$clone(co(0,1)),$mod.TCoordinate.$clone(co(1,1)),$mod.TCoordinate.$clone(co(2,1))];
      $Self.FBlocks[5] = [$mod.TCoordinate.$clone(co(1,0)),$mod.TCoordinate.$clone(co(2,0)),$mod.TCoordinate.$clone(co(0,1)),$mod.TCoordinate.$clone(co(1,1))];
      $Self.FBlocks[6] = [$mod.TCoordinate.$clone(co(0,0)),$mod.TCoordinate.$clone(co(1,0)),$mod.TCoordinate.$clone(co(1,1)),$mod.TCoordinate.$clone(co(2,1))];
    };
    ...
    this.MoveBlockDown = function () {
      ...
      Result = false;
      $Self.Fdirection = 1;
      coll = $Self.CheckForVerticalCollision($Self.Fdirection,$mod.TBlock$clone($Self.FCurBlock));
      Result = coll === 0;
      if (Result) {
        ShiftBlockDown()}
       else {
        if (coll !== 2) ShiftBlockDown();
        $Self.SetGameOver($Self.FCurrPos.y <= 2);
        if (!$Self.FGameOver) {
          for (i = 0; i <= 3; i++) {
            x = $Self.FCurBlock[i].x + $Self.FCurrPos.x;
            y = $Self.FCurBlock[i].y + $Self.FCurrPos.y;
            $Self.FBoard[x][y] = $Self.FCurBlockColor;
          };
    ...
    this.HandleKeyPress = function (k) {
      ...
      Result = false;
      if ($Self.FGameOver) return Result;
      if (k.code === pas.Web.TJSKeyNames.ArrowLeft) {
        DisableKey();
        Result = !$Self.MoveBlockLeftRight(false);
      } else if (k.code === pas.Web.TJSKeyNames.ArrowRight) {
        DisableKey();
        Result = !$Self.MoveBlockLeftRight(true);
...
rtl.module("program",["System","browserapp","JS","Classes","SysUtils","Web","utetris"],function () {
  "use strict";
  var $mod = this;
  rtl.createClass($mod,"TTetrisApplication",pas.browserapp.TBrowserApplication,function () {
    this.$init = function () {
